...
const ModifyBlog = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (id) {
          console.log("Fetching blog from:", `http://localhost:5000/api/admin/blogs/${id}`);
          const blogResponse = await fetch(`http://localhost:5000/api/admin/blogs/${id}`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
          ...
          if (blogResponse.ok) {
            ...
            if (blogData.success && blogData.data) {
              ...
              setForm({
                ...
                resourceLinks: Array.isArray(blog.resourceLinks)
                  ? blog.resourceLinks.map((link) => ({
                      name: link.name || "",
                      url: link.url || "",
                    }))
                  : [],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log(`${id ? "Updating" : "Creating"} blog at:`, url);
      console.log("Blog data:", blogData);
      ...
      if (response.ok && data.success) {
        ...
        throw new Error(data.message || `Failed to ${id ? "update" : "create"} blog`);
      }
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      console.log("Deleting blog:", `http://localhost:5000/api/admin/blogs/${id}`);
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-3xl font-bold text-white mb-2">
                  {id ? "Edit" : "Create"} <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">Blog Post</span>
                </h2>
                <p className="text-gray-300 text-lg">
                  {id ? "Update your blog content" : "Create a new blog post"}
                </p>
              </div>
            </div>
            {id && (
              <motion.button
                ...
                className="mt-4 md:mt-0 px-4 py-2 bg-red-600/20 text-red-300 rounded-xl font-medium hover:bg-red-600/30 transition-all duration-300 flex items-center gap-2 border border-red-500/30"
              >
                  ...
                  {form.title || featuredImage || form.content || form.resourceLinks.length > 0 ? (
                    <div className="space-y-4">
                      ...
                      <div className="inline-flex px-2 py-1 rounded-full text-xs font-medium border"
                        style={{
                          backgroundColor: form.status === "published" ? "rgba(34, 197, 94, 0.2)" : 
                                        form.status === "draft" ? "rgba(234, 179, 8, 0.2)" : 
                                        "rgba(107, 114, 128, 0.2)",
                          borderColor: form.status === "published" ? "rgba(34, 197, 94, 0.3)" :
                                      form.status === "draft" ? "rgba(234, 179, 8, 0.3)" :
                                      "rgba(107, 114, 128, 0.3)",
                          color: form.status === "published" ? "#4ade80" :
                                form.status === "draft" ? "#facc15" :
                                "#9ca3af"
                        }}>
                        {form.status || "draft"}
                      </div>
